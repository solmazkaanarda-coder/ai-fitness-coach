...
type Screen = "welcome" | "account" | "body" | "plan" | "dashboard";
...
function calculateActivityMultiplier(activityLevel: ActivityLevel): number {
...
function calculateTDEE(profile: UserProfile): number {
  ...
  const multiplier = calculateActivityMultiplier(profile.activityLevel);
...
function calculateProteinTarget(profile: UserProfile): number {
  if (profile.goal == "Fat Loss") {
    return Math.round(profile.weightKg * 2.1);
  }
  return Math.round(profile.weightKg * 1.8);
}
function calculateWaterTarget(profile: UserProfile): number {
  return Math.round(profile.weightKg * 35);
}
function calculateStepTarget(profile; UserProfile): number {
  if (profile.goal === "Fat Loss") {
    return 10000;
  }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.page}>
        ...
        <Text style={styles.subtitle}>
          Your real-time fitness plan 👇
        </Text>

        <View style={styles.grid}>
          <Text style={styles.cardTitle}>Calories: {calories}</Text>
          <Text style={styles.cardTitle}>Protein: {protein}g</Text>
          <Text style={styles.cardTitle}>Water: {water}ml</Text>
          <Text style={styles.cardTitle}>Steps: {steps}</Text>
        </View>
